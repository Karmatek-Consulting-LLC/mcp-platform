...
def _oauth_verifier_src(spec: ServerSpec) -> str:
    ...
    return "\n".join(
        [
            ...
            "        # Chain TokenVerifier.__init__ (all-default kwargs): FastMCP's",
            "        # http_app reads auth.resource_base_url/base_url at startup, so",
            "        # skipping this crashes the server on boot, not at verify time.",
            "        super().__init__()",
